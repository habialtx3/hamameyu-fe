  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fafafa] text-gray-800">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-0 lg:divide-x lg:divide-gray-300">
          ...
          <div className="lg:pr-12 flex flex-col">
            ...
            <div className="space-y-4 mb-8 text-base">
              <p><span className="text-black font-semibold">Nama pelapor:</span> Warga (User ID: {report.user_id})</p>
              <p><span className="text-black font-semibold">Nomor Laporan:</span> #{report.id}</p>
              <p><span className="text-black font-semibold">Kategori:</span> <span className="capitalize">{report.category}</span></p>
              <p>
              ...
              <p><span className="text-black font-semibold">Koordinat Lokasi:</span> Lat {report.location.latitude}, Lng {report.location.longitude}</p>
            </div>

            {/* Detail Keluhan Box */}
            <div className="relative bg-gray-50/50 border border-gray-300 rounded-2xl p-6 mb-8 mt-2">
              <span className="absolute -top-3 left-6 bg-white px-2 text-xs text-gray-400">Detail Keluhan</span>
              <h3 className="font-bold text-black mb-1 text-base">{report.title}</h3>
              <p className="text-black text-sm md:text-base leading-relaxed">{report.description}</p>
            </div>
                  ...
                  report.images.map((imgUrl, idx) => {
                    ...
                    return (
                      <img
                        ...
                        className="w-full aspect-square object-cover rounded-3xl border border-gray-200 cursor-pointer hover:opacity-90 transition duration-200"
                        onClick={() => {
          ...
          <div className="lg:pl-12 flex flex-col">
            ...
            <div className="bg-[#f4f5f6] border border-gray-200 rounded-[2rem] p-8 md:p-10 flex-1">
              <div className="flex flex-col items-center mb-10">
                <img src="/assets/illust/on_progress.png" alt="Ilustrasi Progress" className="w-full max-w-sm object-contain mb-4 rounded-xl" />
                <p className="text-gray-700 text-sm font-semibold tracking-wide capitalize">Status: {statusText}</p>
              </div>
              ...
              <div className="relative border-l border-gray-300 ml-5 space-y-10">
                ...
                <div className="relative pl-10">
                  ...
                  <p className="text-[13px] text-gray-500 mt-1.5 leading-relaxed">Sistem telah menerima laporanmu. Keluhan berhasil diarsip ke dalam database dengan ID #{report.id}.</p>
                </div>

                {/* Step 2 */}
                <div className="relative pl-10">
                  <div className={`absolute -left-[1px] -top-10 w-[1px] h-10 ${report.status !== "pending" ? "bg-[#4ca64c]" : "bg-gray-300"}`}></div>
                  <div className={`absolute -left-[18px] top-0 w-9 h-9 rounded-full flex items-center justify-center ring-8 ring-[#f4f5f6] ${report.status !== "pending" ? "bg-[#4ca64c]" : "bg-gray-300 text-gray-400"}`}>
                    <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                  </div>
                  <h4 className={`font-bold text-[15px] ${report.status !== "pending" ? "text-black" : "text-gray-400"}`}>Validasi & Verifikasi Admin</h4>
                  <p className="text-[13px] text-gray-500 mt-1.5 leading-relaxed">
                    {report.status !== "pending" ? "Laporanmu sudah ditinjau oleh petugas RT/RW dan saat ini sedang masuk tahap penanganan lapangan." : "Menunggu antrean verifikasi dari petugas admin setempat."}
                  </p>
                </div>
                ...
                <div className="relative pl-10">
                  ...
                  <div className={`absolute -left-[18px] top-0 w-9 h-9 rounded-full flex items-center justify-center ring-8 ring-[#f4f5f6] ${report.status === "done" ? "bg-[#4ca64c]" : "bg-gray-300 text-gray-400"}`}>
                    <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                  </div>
                  <h4 className={`font-bold text-[15px] ${report.status === "done" ? "text-black" : "text-gray-400"}`}>Masalah Selesai Ditangani</h4>
                  <p className="text-[13px] text-gray-500 mt-1.5 leading-relaxed">
                    {report.status === "done" ? `Keluhan telah selesai diperbaiki pada ${report.time_close ? new Date(report.time_close).toLocaleDateString("id-ID") : "hari ini"}. Terima kasih atas laporan Anda!` : "Tahap penyelesaian akhir setelah penanganan fisik selesai dilakukan."}
                  </p>
                </div>
              </div>
            </div>
      ...
      <ViewImage
        isOpen={isModalOpen}
        src={selectedImg}
        alt="Detail Bukti Laporan Keluhan"
